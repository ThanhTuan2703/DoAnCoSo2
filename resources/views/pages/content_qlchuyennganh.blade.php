                ...
                <div class="row">
                    <div class="col-xl-12">
                        <div class="tatca them11">
                            ...
                            <div class="themmoi "><button><a href="{{ route('chuyennganh.create') }}"
                    ...
                    <div class="table-responsivess">
                        ...
                        <table class="table table_res">
                            ...
                            <tbody>
                                @foreach ($data as $row)
                                    <tr>
                                        <td>{{ $row->tenchuyennganh }}</td>
                                        <td>{{ $row->machuyennganh }}</td>
                                        <td>{{ $row->khoa }}</td>
                                        <td>{{ $row->mota }}</td>
                                        <td style="display: flex">
                                            <a href="{{ route('chuyennganh.edit', $row->machuyennganh) }}" title="Edit"
                                                class="edit"><i class="fa-solid fa-pen-to-square"></i></a>
                                            <form method="POST"
                                                action="{{ route('chuyennganh.destroy', $row->machuyennganh) }}">
                                                @csrf
                                                @method('delete')
