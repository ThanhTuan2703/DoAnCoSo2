@extends('index')

@section('content')
    <link rel='stylesheet' href='https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css'>
    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/animate.css/3.7.0/animate.min.css'>
    <link rel='stylesheet' href='https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css'>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css">
    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/tether/1.4.4/css/tether.min.css'>
    <link rel="stylesheet" href="{{ asset('resources/css/style.css') }}">
    <link rel="stylesheet" href="{{ asset('resources/css/quanlikhoa.css') }}">
    <link rel="stylesheet" href="{{ asset('resources/css/table.css') }}">
    <link rel="stylesheet" href="{{ asset('responsive_table.css') }}">
    </head>

    <div class="">
        <div class="container-fluid">
            <div class="row">
                <div class="col-xl-12">
                    <div class="tatca them11">
                        <div class="ngoinha"><a href=""><i class="fa-solid fa-house-user text-light"></i></a></div>
                        <div class="hedaotao">Trang chủ</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="container pt-5">
            <div class="row">
                <div class="col-12 boder-table">
                    <div class="table-responsivess">
                        <div class="row mt-2">
                            <div class="col-lg-12">
                                <h2 style="display: flex; height: 34px; align-items: center; font-size:25px">Thêm mới chuyên
                                    nghành</h2>
                            </div>
                        </div>
                    </div>
                    @if (\Session::has('abc'))
                        <div class="alert alert-secondary" style="color: red; text-align: center">
                            {{ \Session::get('abc') }}
                        </div>
                    @endif
                    <form action="{{ route('chuyennganh.store') }}" method="post">
                        @csrf
                        <div class="container-xl">
                            <div class="row mt-4">
                                <div class="col-lg-2">
                                    <label for="">Khoa</label>
                                </div>
                                <div class="col-lg-10">
                                    <select name="sokhoa" style="width:35%">
                                        @foreach ($travetenkhoa as $travetenkhoa)
                                            <option value="{{ $travetenkhoa->tenkhoa }}">{{ $travetenkhoa->tenkhoa }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('sokhoa'))
                                        <span class="text-danger">{{ $errors->first('sokhoa') }}</span>
                                    @endif
                                </div>
                            </div>

                            <div class="row mt-4">
                                <div class="col-lg-2">
                                    <label for="">Chuyên ngành</label>
                                </div>
                                <div class="col-lg-10">
                                    <input name="tenchuyennganh" style="width: 40%" type="text"
                                        value="VD : Kỹ thuật phần mềm">
                                    @if ($errors->has('tenchuyennganh'))
                                        <span class="text-danger">{{ $errors->first('tenchuyennganh') }}</span>
                                    @endif
                                </div>
                            </div>

                            <div class="row mt-4">
                                <div class="col-lg-2">
                                    <label for="">Mã chuyên ngành</label>
                                </div>
                                <div class="col-lg-10">
                                    <input name="machuyennganh" style="width: 20%" type="text"><br>
                                    @if ($errors->has('machuyennganh'))
                                        <span class="text-danger">{{ $errors->first('machuyennganh') }}</span>
                                    @endif
                                </div>

                            </div>

                            <div class="row mt-4">
                                <div class="col-lg-2">
                                    <label>Mô tả</label>
                                </div>
                                <div class="col-lg-10">
                                    <textarea id="" name="motachuyennganh" style="width: 99%;"></textarea>
                                    @if ($errors->has('motachuyennganh'))
                                        <span class="text-danger">{{ $errors->first('motachuyennganh') }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>

                        <div class="Dep-form d-flex justify-content-center mt-4">
                            <div class="mr-3">
                                <input type="submit" value="Lưu" class="btn btn-dark">
                            </div>
                            <div class="mr-3">
                                <a href="{{ route('chuyennganh.create') }}" class="btn btn-primary ">Làm mới</a>
                            </div>
                            <div class="mr-3">
                                <a href="{{ route('chuyennganh.chuyennganh') }}" class="btn btn-danger ">Trở về</a>
                            </div>
                        </div>
                </div>
            </div>
        </div>

    </div>
    </div>
    </div>

    <script src='https://code.jquery.com/jquery-3.3.1.slim.min.js'></script>
    <script src='https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js'></script>
    <script src='https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js'></script>
    <script src='https://cdnjs.cloudflare.com/ajax/libs/tether/1.4.4/js/tether.min.js'></script>
    {{-- <script  src="{{('resources/js/script.js')}}"></script> --}}
    {{-- href="{{('public/backend/css/style.css')}}" --}}
    <script src="{{ asset('resources/js/script.js') }}"></script>
@endsection
